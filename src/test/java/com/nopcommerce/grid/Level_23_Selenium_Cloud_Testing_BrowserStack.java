...
public class Level_23_Selenium_Cloud_Testing_BrowserStack extends BaseTest{
	...
	@Parameters({"browser","environmentName","osName","osVersion"})
	@BeforeClass
	public void beforeClass(String browserName,String environmentName,String osName,String osVersion) {
		...
		ConfigFactory.setProperty("env",environmentName); //Hàm này sẽ map value=dev,staging,test,... của global parameter trong file .xml vào trong biến env của interface Environment tại line số 5 để có dc tên file như dev.properties hay staging.properties
		env = ConfigFactory.create(Environment.class);
		driver = getBrowserDriverCloudBrowserStack(browserName,env.appURL(),osName,osVersion); //Sau đó tại đây ta gọi env.appURL() để lấy url của môi trường cần test và các env.username,env.pass,....cần thiết đã config trong file Environment.java và dev.properties(vì trong Environment.java(là interface) dùng lib Owner để map biến cần gọi tương ứng với value trong dev.properties
