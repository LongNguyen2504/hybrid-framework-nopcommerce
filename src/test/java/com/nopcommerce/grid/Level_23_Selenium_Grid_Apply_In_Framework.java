...
public class Level_23_Selenium_Grid_Apply_In_Framework extends BaseTest{
	...
	@Parameters({"browser","environmentName","osName","ipAddess","port"})
	@BeforeClass
	public void beforeClass(String browserName,String environmentName,String osName,String ipAddress,String portNumber) {
		...
		driver = getBrowserDriverGrid(browserName,env.appURL(),osName,ipAddress,portNumber); //Sau đó tại đây ta gọi env.appURL() để lấy url của môi trường cần test và các env.username,env.pass,....cần thiết đã config trong file Environment.java và dev.properties(vì trong Environment.java(là interface) dùng lib Owner để map biến cần gọi tương ứng với value trong dev.properties
