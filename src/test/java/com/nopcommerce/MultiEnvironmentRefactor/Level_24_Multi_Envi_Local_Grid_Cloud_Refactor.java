package com.nopcommerce.MultiEnvironmentRefactor;
...
//Class này demo cho cả phần manage data test và data driven test + cloudtesting browserStack
//Apply POM cho test case

public class Level_24_Multi_Envi_Local_Grid_Cloud_Refactor extends BaseTest{
	...
	private Environment env; //Staging,Test,Product,Dev,...

	@Parameters({"envName","serverName","browser","ipAddress","portNumber","osName","osVersion"})
	@BeforeClass
	public void beforeClass(@Optional("local") String envName, @Optional("dev") String serverName,@Optional("chrome") String browser,@Optional("localhost") String ipAddress,@Optional("4444") String portNumber,@Optional("Windows") String osName,@Optional("10") String osVersion) { // dùng @Optional khi tham số đó ko dc khai báo và truyền vào từ xml thì tại beforeClass này tham số đó vẫn có giá trị để tránh beforeClass bị lỗi thiếu tham số để dùng tại hàm getBrowserDriverFinal() vì bắt buộc hàm getBrowserDriverFinal() phải có đủ 7 tham số thì mới work dc cho nhiều môi trường như local,grid,cloud,..tùy vào testcase muốn run môi trường nào thì truyền giá trị môi trường vào parameter global envName bên file .xml

		//Lưu ý tham số envName = local,grid,browserStack,Saucelab,Lambda,Crossbrowser,... / còn serverName = TEST,STAGING,DEV,... mà ở các class trước đã gọi serverName = envName
		//Demo cách dùng file .properties để get biến environment từ file src/test/resources/dev.properties hoặc test.properties
		ConfigFactory.setProperty("env",serverName); //Hàm này sẽ map value=dev,staging,test,... của global parameter trong file .xml vào trong biến env của interface Environment tại line số 5 để có dc tên file như dev.properties hay staging.properties
		env = ConfigFactory.create(Environment.class); // get serverName và url tương ứng của serverName đó
		//Demo của Level_24_Multi_Envi_Local_Grid_Cloud_Refactor
		driver = getBrowserDriverFinal(browser,env.appURL(),envName,ipAddress,portNumber,osName,osVersion);

		System.out.println("appUserName: " + env.appUserName());
